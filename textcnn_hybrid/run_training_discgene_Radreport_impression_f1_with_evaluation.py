...
flags.DEFINE_string('out_dir', '/ceph/chpc/shared/aristeidis_sotiras_group/panxiao_scratch/large-scale-comparison-setting4/data_rad/two/textcnn_hybrid/Radreport_Impression_f1/output', 'Output directory.')
...
save_dir = '/ceph/chpc/shared/aristeidis_sotiras_group/panxiao_scratch/large-scale-comparison-setting4/data_rad/two/textcnn_hybrid/Radreport_Impression_f1/checkpoints/textcnn_impression/' + FLAGS.dataset + '_' + FLAGS.w2v_source + '_epochs' + str(FLAGS.epochs) + '_bs' + str(FLAGS.batch_size) + '_bl' \
...
def train(hybridmodel, x_train, y_train, wordpairs_train, x_dev, y_dev, wordpairs_dev, x_test, y_test, wordpairs_test, x_arash, y_arash, wordpairs_arash, x_mohamed, y_mohamed, wordpairs_mohamed, x_ps, y_ps, wordpairs_ps):
    print("Configuring TensorBoard and Saver...", flush=True)
    # 配置 Tensorboard，重新训练时，请将tensorboard文件夹删除，不然图会覆盖
    tensorboard_dir = 'tensorboard/Textcnn_Impression_' + FLAGS.dataset + '_' + FLAGS.w2v_source + '_epochs' + str(FLAGS.epochs) + '_bs' + str(FLAGS.batch_size) + '_bl' \
           + str(FLAGS.balance_lambda) + '_bf' + str(FLAGS.balance_function)  + '_l2' + str(FLAGS.l2_reg_lambda) + '_fold' + str(FLAGS.fold) + '_iterations' + str(FLAGS.iterations)
    ...
    require_improvement = FLAGS.iterations  # 如果超过1000轮未提升，提前结束训练
        ...
        for x_batch, y_batch, wordpairs_batch, ith_batch, num_batch in batch_train:
            ...
            if total_batch % FLAGS.print_per_batch == 0:
                ...
                if prfs[2] > best_f1_val :
                    ...
                    loss_arash, acc_arash, prfs_arash, cf_arash, y_preds_best_arash, y_trues_best_arash, y_probs_best_arash = evaluate(hybridmodel, session, x_arash, y_arash, wordpairs_arash)
                    loss_mohamed, acc_mohamed, prfs_mohamed, cf_mohamed, y_preds_best_mohamed, y_trues_best_mohamed, y_probs_best_mohamed = evaluate(hybridmodel, session, x_mohamed, y_mohamed, wordpairs_mohamed)
                    loss_ps, acc_ps, prfs_ps, cf_ps, y_preds_best_ps, y_trues_best_ps, y_probs_best_ps = evaluate(hybridmodel, session, x_ps, y_ps, wordpairs_ps)
                    ...
                    print('####################################\n\n')
                    print('Arash:')
                    print("Binary_precision_recall_fscore_support:" + str(prfs_arash))
                    print('Confusion Matrix:\n', cf_arash)
                    confidence_interval(y_trues_best_arash, y_preds_best_arash)
                    print('####################################\n\n')
                    print('Mohamed:')
                    print("Binary_precision_recall_fscore_support:" + str(prfs_mohamed))
                    print('Confusion Matrix:\n', cf_mohamed)
                    confidence_interval(y_trues_best_mohamed, y_preds_best_mohamed)
                    print('####################################\n\n')
                    print('Prospective Study:')
                    print("Binary_precision_recall_fscore_support:" + str(prfs_ps))
                    print('Confusion Matrix:\n', cf_ps)
                    confidence_interval(y_trues_best_ps, y_preds_best_ps)
                else:
                    improved_str = ''
...
def main(_):
  ...
  # External Evaluation Arash
  arash_path = '/ceph/chpc/shared/aristeidis_sotiras_group/panxiao_scratch/large-scale-comparison-setting4/data_rad/External Evaluation/mimic-cxr-impression-Arash.txt' 
  arash_E = read_external_dataset_impression(arash_path, fold)
  x_arash, y_arash= load_data_labels(arash_E)

  # External Evaluation Mohamed
  mohamed_path = '/ceph/chpc/shared/aristeidis_sotiras_group/panxiao_scratch/large-scale-comparison-setting4/data_rad/External Evaluation/mimic-cxr-impression-Mohamed.txt' 
  mohamed_E = read_external_dataset_impression(mohamed_path, fold)
  x_mohamed, y_mohamed = load_data_labels(mohamed_E)

  # Prospective Study
  ps_path = '/ceph/chpc/shared/aristeidis_sotiras_group/panxiao_scratch/large-scale-comparison-setting4/data_rad/Prospective study/prospective_study.txt' 
  ps_E = read_external_dataset_impression(ps_path, fold)
  x_ps, y_ps = load_data_labels(ps_E)
  ...
  if FLAGS.w2v_source == "google":
    ...
    # Read Word Vectors
    word_vector_file = './files/output_impression/' + str(FLAGS.embed_size) + '/' + 'model'
  ...
  x_arash, wordpairs_arash = w2vdataset.get_wordpairs(x_arash, FLAGS.min_count, max_document_length)
  x_mohamed, wordpairs_mohamed = w2vdataset.get_wordpairs(x_mohamed, FLAGS.min_count, max_document_length)
  x_ps, wordpairs_ps = w2vdataset.get_wordpairs(x_ps, FLAGS.min_count, max_document_length)
  ...
  dirembed = os.path.join(FLAGS.out_dir, str(FLAGS.embed_size)) + '/' + FLAGS.w2v_source + '/epochs' + str(
      ...
      + '/iterations' + str(FLAGS.iterations)
  print(dirembed)
  ...
  if sys.argv[1] == 'train':
      print('-----------5-------------', flush=True)
      syn0_final, y_preds_best, y_trues_best, y_probs_best = train(hybridmodel, x_train, y_train, wordpairs_train, x_dev, y_dev, wordpairs_dev, x_test, y_test, wordpairs_test, x_arash, y_arash, wordpairs_arash, x_mohamed, y_mohamed, wordpairs_mohamed, x_ps, y_ps, wordpairs_ps)
  print('-----------6-------------', flush=True)
