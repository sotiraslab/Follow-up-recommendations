...
class Word2VecDataset(object):
  ...
  def _build_raw_vocab(self, filenames):
    ...
    #topnoun = []
    #for w, count in raw_vocab:
    #  if wn.synsets(w) and wn.synsets(w)[0].pos()=='n':
    #     topnoun.append(w)
  ...
  def get_tensor_dict(self, filenames):
    ...
    dataset = tf.data.Dataset.zip((
        tf.data.TextLineDataset(filenames).repeat(self._epochs),     #read the whole file and then repeat several(_epoch) times.
...
def generate_instances(indices, arch, window_size, codes_points=None):
  ...
  def per_target_fn(index, init_array):
    ...
    context = tf.concat([left, right], axis=0)   ####这里求出来的相当于索引，还要把索引对应的元素取出来
    context = tf.gather(indices, context)        ####把索引对应的元素取出来
  ...
  _, result_array = tf.while_loop(lambda i, ta: i < size,
                                  ...
                                  [0, init_array],
                                      back_prop=False)
  #tf.Print(result_array, [result_array], summarize=200)
  instances = tf.to_int64(result_array.concat())
  #tf.Print(instances, [instances], summarize=200)
  return instances
