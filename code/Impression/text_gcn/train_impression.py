...
if len(sys.argv) != 2:
	sys.exit("Use: python train_impression.py <dataset>")
...
# Load data
adj, features, y_train, y_val, y_test, train_mask, val_mask, test_mask, train_size, test_size = load_corpus_impression(
    FLAGS.dataset)
print(adj)
# print(adj[0], adj[1])
...
save_path = os.path.join(save_dir, 'best_validation_impression')
for epoch in range(FLAGS.epochs):
    ...
    # Training step
    outs = sess.run([model.opt_op, model.loss, model.accuracy, model.pred, model.labels,
    ...
    if prfs_val[2] > best_f1_val:
        ...
        np.save('./Impression/prob_preb_impression', probability_preb)
        ...
        confidence_interval(labels_test, pred_test)
    ...
    print("Epoch:", '%04d' % (epoch + 1), "train_loss=", "{:.5f}".format(outs[1]),
          ...
          "val_acc=", "{:.5f}".format(acc), "f1_val", "{:.5f}".format(prfs_val[2]), "time=", "{:.5f}".format(time.time() - t))
