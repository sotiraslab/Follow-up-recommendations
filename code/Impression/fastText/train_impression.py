#conda environment:word2vecpy
import fasttext
from sklearn.metrics import confusion_matrix, precision_recall_fscore_support, accuracy_score
import numpy as np

model = fasttext.train_supervised(input='./Impression/Train/Radreport.train', lr = 0.1, epoch = 10, minCount = 5 , dim=300, autotuneValidationFile='./Impression/Validation/Radreport.validation')

test_list = open("./Impression/Test/Radreport.test",'r')
...
model.save_model("ft_impression_300.bin")
test_list.close()
proba_labels = np.array(proba_labels)
np.save('fasttext_impression_300_w_modelsaved.npy', proba_labels)
